...
**当前授权：** 用户新接管指令已取代历史逐关审批及全局 G00/G03/G04 开发冻结规则。总控按真实证据推进已授权工作，仅真实高风险或公开/预算决策请用户确认；历史未通过结果保留。
...
## 后台脚本续传（2026-09-06，当前有效）

用户要求“把断点续传的工作交给脚本进行，估算大致完成时间”，因此上节“不自动循环重试／无下载进程”为启动前历史。新增唯一操作脚本 [scripts/resume-orca-download.ps1](../scripts/resume-orca-download.ps1)，仅负责同一官方资产的续传与校验，不是 Agent 调度器，不进行安装。

- 实际启动：本地 `2026-09-06 18:05:41 +08:00`，隐藏窗口、独立 PowerShell 进程，PID `42656`；起始分片 10,320,256 字节。使用 `Start-Process`，无计划任务、服务或开机启动项。
- 写入范围：仅已授权实验下载目录的 `.part`、最终 AppImage、`resume-download.log` 与用于互斥的 `resume-download.lock`；不改日常 Orca/home/代理。锁是本地文件独占句柄，退出即释放，保留空文件不代表仍运行。
- 每次 curl 最长 300 秒，保留断点；可重试错误间隔 15 秒，总传输窗口最多 4 小时（约至 22:05:41）；连续 3 次无进展或不可重试错误则停止。日志保留退出码、HTTP 状态和字节数，不记录完整网络响应、签名重定向 URL 或凭据。
- 只有完整大小为 205918977 且官方 SHA256 相符才改名为 `orca-linux.AppImage`；不覆盖现有正式文件，不自动执行、安装或启动 Orca。
- 实际验证：PowerShell Parser 无语法错误；首次后台运行后文件增长；再次启动脚本退出 1，互斥阻止重复写入，日志只有一个 START。完整下载后的校验/改名分支尚未实跑，未声称完成。
- 时间估算：18:06:23 实测分片 11,348,352 字节，启动后 42.4 秒新增 1,028,096 字节，平均约 23.7 KiB/s。按剩余字节/实测速率并计入约 5% 的重试间隔，约还需 140 分钟，即北京时间 20:27 左右；可按 20:10–20:45 作为粗略范围，不是完成承诺。

查看状态：读取 `C:\Users\DW\AppData\Local\OrcaKernelLab\downloads\v1.4.188\resume-download.log`，并核对 `.part` 实时大小。`COMPLETE` 才表示官方摘要已通过，`STOPPED` 表示已退出并需看原因；电脑休眠或断网会影响预计时间。进程正在运行时无需再次启动脚本。

当前关卡仍为 G00 待验收；正在运行的是下载脚本，没有 Worker 或新容器。安装、serve 与 G03 隔离验收均未完成；无需新的用户决定。
